...
export const DetailBottomSheetContent = ({
  ...
  const mockMovieData = {
    ...
    country: '미국',
    synopsis:
      '애니메이션 「주토피아(Zootopia)」는 다양한 동물들이 인간처럼 살아가는 현대적인 도시 ‘주토피아’를 배경으로, 토끼 주인공 ‘주디 홉스’가 경찰이 되겠다는 꿈을 이루기 위해 고군분투하는 이야기다. 포식자와 초식자가 평화롭게 공존하는 이상적인 도시처럼 보이지만, 그 이면에는 차별과 편견, 고정관념이 존재한다. 주디는 여우 사기꾼 ‘닉 와일드’와 협력해 동물들이 사라지는 미스터리한 사건을 추적하며, 사건의 배후에 있는 진실을 밝혀낸다. 이 과정에서 둘은 서로에 대한 편견을 극복하고 진정한 파트너로 성장한다. 영화는 흥미진진한 추리와 코믹한 요소를 결합하면서도, 다양성과 편견, 사회적 갈등 같은 주제를 섬세하게 다뤄 깊은 울림을 준다. 어린이와 성인 모두에게 의미 있는 메시지를 전하는 작품으로, 비주얼과 스토리, 캐릭터의 조화가 돋보이는 디즈니의 대표 애니메이션 중 하나다.',
  ...
  return (
    <div className="flex flex-col overflow-y-auto scrollbar-hide">
      ...
      {/* 사진 밑에 콘텐츠 정보 */}
      <div className="flex flex-col space-y-6 px-4 pt-4 bg-gradient-to-b from-black/100 to-black/0">
        {/* "보러가기" 버튼 */}
        <PlatformButton
          platformName="디즈니+"
          iconUrl={getPlatformLogo('disney plus') || ''}
